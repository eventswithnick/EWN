(function () {
  ...
  async function refreshRemoteBookings() {
    if (!hasRemote()) return false;
    try {
      // Do not order by the mixed-case dateCreated column in the REST URL.
      // PostgREST can treat it as lowercase datecreated and reject the request.
      ...
      console.warn('Remote booking load failed:', error.message);
      const status = document.getElementById('connectionStatus');
      if (status) status.textContent = 'Database connected, but bookings could not load';
      return false;
    }
  }
