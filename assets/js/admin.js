(function () {
  ...
  async function verifyAdminAccess() {
    ...
    try {
      const result = await supabaseRequest('rpc/is_events_with_nick_admin', 'POST', {}, null);
      if (result === true || result === 'true') return true;
      const email = (currentAdmin && currentAdmin.email) || (getStoredSession() && getStoredSession().user && getStoredSession().user.email) || 'this admin email';
      throw new Error(`The login is valid, but ${email} is not approved in the admin_users table.`);
    } catch (error) {
      throw new Error(`${error.message} Add the exact Supabase Auth email to public.admin_users, then refresh the admin portal.`);
    }
  }
  ...
  async function refreshRemoteBookings() {
    ...
    try {
      ...
      if (Array.isArray(rows)) { setBookings(rows); return true; }
      return false;
    } catch (error) {
      console.warn('Remote booking refresh failed:', error.message);
      systemNotice(`Bookings could not be loaded from Supabase. ${sanitize(error.message)}<br><small>Most common cause: the logged-in email is not in the admin_users table, or the v7.2 SQL policies were not run.</small>`, 'error');
      return false;
    }
  }
  async function refreshRemoteConfig() {
    ...
    if (!ok) systemNotice('Some admin data could not be loaded from Supabase. Check that the admin email is approved and the v7.2 SQL fix has been run.', 'warn');
    return ok;
  }
  async function upsertRemote(table, body) {
    ...
    try {
      ...
      systemNotice(`${sanitize(table)} could not be saved to Supabase. ${sanitize(error.message)}`, 'error');
      return false;
    }
  }
  ...
  function setLoginModeText() {
    ...
    if (hasRemote()) {
      ...
      submitBtn.textContent = 'Open Secure Portal';
      note.innerHTML = 'Secure live mode: log in using the Supabase Auth admin email and password.';
    } else {
      emailField.classList.add('hidden');
      submitBtn.textContent = 'Open Local Demo Portal';
      note.innerHTML = 'Local presentation mode: use the presentation password only for offline testing.';
    }
  }
  ...
  async function showDashboard() {
    ...
    if (status) status.textContent = hasRemote() ? `Secure database connected${currentAdmin && currentAdmin.email ? ': ' + currentAdmin.email : ''}` : 'Presentation mode (local only)';
    systemNotice('', 'info');
    if (hasRemote()) {
      try {
        await verifyAdminAccess();
        systemNotice('Secure admin access confirmed. Bookings, prices, banking details and website settings can now sync with Supabase.', 'success');
      } catch (error) {
        systemNotice(`${sanitize(error.message)}<br><small>Run the ADMIN_ACCESS_FIX SQL in Supabase using the same email you used to log in.</small>`, 'error');
      }
    }
  ...
  function initLogin() {
    ...
    form.addEventListener('submit', async event => {
      ...
      button.textContent = hasRemote() ? 'Checking secure login...' : 'Checking password...';
      try {
        ...
        button.textContent = hasRemote() ? 'Open Secure Portal' : 'Open Local Demo Portal';
      }
    });
  ...
  function initBookingControls() {
    ...
    document.getElementById('addSampleBtn').addEventListener('click', showAddBookingForm);
  }
  ...
  async function createManualBooking(data) {
    ...
    if (hasRemote()) {
      try {
        ...
        systemNotice('Manual booking saved to Supabase.', 'success');
        await refreshRemoteBookings();
        renderAll();
      } catch (error) {
        systemNotice(`Manual booking was saved locally but could not be saved to Supabase. ${sanitize(error.message)}`, 'error');
      }
    }
  }
  ...
  function initPricing() {
    ...
    form.addEventListener('click', e => {
      const addGroup = e.target.dataset.priceAdd;
      const removeGroup = e.target.dataset.priceRemove;
      ...
      if (removeGroup) {
        const index = Number(e.target.dataset.priceIndex);
        if (pricing[removeGroup]) pricing[removeGroup].splice(index, 1);
      }
      setPricing(pricing);
      renderPricingForm();
      populateQuoteSelects();
      document.getElementById('pricingSaved').textContent = 'Unsaved price changes. Click Save Price Settings to sync online.';
    });
    document.getElementById('savePricingBtn').addEventListener('click', async () => {
      ...
      document.getElementById('pricingSaved').textContent = hasRemote() ? (savedRemote ? 'Price settings saved locally and to Supabase. Quote builder will now use the updated prices.' : 'Price settings saved locally, but Supabase rejected the online save. Check admin access.') : 'Price settings saved. Quote builder will now use the updated prices.';
    });
  ...
  function initSettings() {
    ...
    form.addEventListener('submit', async event => {
      ...
      document.getElementById('settingsSaved').textContent = hasRemote() ? (savedRemote ? 'Website details saved locally and to Supabase.' : 'Website details saved locally, but Supabase rejected the online save.') : 'Website details saved. Refresh the public website to see changes.';
    });
  }
  ...
  function initBanking() {
    ...
    form.addEventListener('submit', async event => {
      ...
      document.getElementById('bankingSaved').textContent = hasRemote() ? (savedRemote ? 'Banking details saved locally and to Supabase.' : 'Banking details saved locally, but Supabase rejected the online save.') : 'Banking details saved. Quote PDF and quote text will use these details.';
    });
  }
  ...
  function initBackup() {
    const refreshBtn = document.getElementById('refreshSupabaseBtn');
    if (refreshBtn) refreshBtn.addEventListener('click', async () => {
      if (!hasRemote()) return systemNotice('Supabase is not configured. The admin portal is in local mode.', 'warn');
      refreshBtn.disabled = true; refreshBtn.textContent = 'Refreshing...';
      await refreshRemoteConfig();
      await refreshRemoteBookings();
      renderAll();
      refreshBtn.disabled = false; refreshBtn.textContent = 'Refresh Supabase';
    });
