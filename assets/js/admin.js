(function () {
  ...
  const keys = {
    settings: 'ewn_site_settings_v7',
    services: 'ewn_services_v7',
    gallery: 'ewn_gallery_v7',
    packages: 'ewn_packages_v7',
    quotePricing: 'ewn_quote_pricing_v7',
    banking: 'ewn_banking_details_v7',
    bookings: 'ewn_bookings_v7',
    auth: 'ewn_admin_supabase_session_v7',
    localAuth: 'ewn_local_admin_auth_v7'
  };
  let currentBookingId = null;
  let lastQuoteText = '';
  let currentAdmin = null;
  const statuses = ['New', 'Contacted', 'Quoted', 'Confirmed', 'Completed', 'Cancelled'];
  let bookingsLoadState = 'idle';
  let bookingReloadPromise = null;

  function systemNotice(message, type = 'info') {
    let box = document.getElementById('adminSystemNotice');
    if (!box) {
      box = document.createElement('div');
      box.id = 'adminSystemNotice';
      box.className = 'notice-box';
      const dashboard = document.getElementById('dashboard');
      const firstPanel = dashboard ? dashboard.querySelector('.tab-panel') : null;
      if (dashboard && firstPanel) dashboard.insertBefore(box, firstPanel);
    }
    if (!message) { box.className = 'notice-box hidden'; box.textContent = ''; return; }
  ...
  function apiBase() { return backend.supabaseUrl.replace(/\/$/, ''); }
  function getStoredSession() {
    try { return JSON.parse(sessionStorage.getItem(keys.auth) || 'null'); }
    catch (error) { console.warn('Could not read secure session:', error.message); return null; }
  }
  function storeSession(session) {
    if (session && session.expires_in && !session.expires_at) session.expires_at = Math.floor(Date.now() / 1000) + Number(session.expires_in);
    sessionStorage.setItem(keys.auth, JSON.stringify(session));
  }
  function clearSession() { sessionStorage.removeItem(keys.auth); sessionStorage.removeItem(keys.localAuth); currentAdmin = null; }
  function getAccessToken() { const session = getStoredSession(); return session && session.access_token ? session.access_token : ''; }
  ...
  function write(key, value) {
    ...
    catch (error) { console.warn('Storage write failed:', key, error); }
  }
  ...
  function normalisePricing(value) {
    const base = clone(defaults.quotePricing || {});
    const incoming = value && typeof value === 'object' ? value : {};
    ['serviceLevels', 'decorLevels', 'cateringOptions', 'extras'].forEach(group => {
      if (Array.isArray(incoming[group]) && incoming[group].length) {
        base[group] = incoming[group].map((item, index) => ({
          id: item.id || `${group}-${index + 1}`,
          label: item.label || 'New item',
          amount: Number(item.amount || 0),
          ...(group === 'extras' ? { enabled: item.enabled !== false } : {})
        }));
      }
      if (!Array.isArray(base[group])) base[group] = [];
    });
    if (incoming.perGuestCoordination !== undefined && incoming.perGuestCoordination !== null && incoming.perGuestCoordination !== '') {
      base.perGuestCoordination = Number(incoming.perGuestCoordination || 0);
    }
    if (base.perGuestCoordination === undefined) base.perGuestCoordination = 0;
    return base;
  }

  async function supabaseAuth(path, method = 'GET', body) {
    if (!hasRemote()) throw new Error('Supabase is not configured yet.');
    const token = getAccessToken();
    const headers = { apikey: backend.supabaseAnonKey, 'Content-Type': 'application/json' };
    ...
    if (!response.ok) throw new Error(payload.error_description || payload.msg || payload.message || `Supabase Auth failed: ${response.status}`);
    return payload;
  }

  async function signInAdmin(email, password) {
    const session = await supabaseAuth('token?grant_type=password', 'POST', { email, password });
    if (!session || !session.access_token) throw new Error('Login failed. Check the admin email and password.');
    storeSession(session);
    currentAdmin = session.user || null;
  }
  ...
  async function ensureFreshSession() {
    ...
    const refreshed = await supabaseAuth('token?grant_type=refresh_token', 'POST', { refresh_token: session.refresh_token });
    storeSession(refreshed);
    currentAdmin = refreshed.user || currentAdmin;
  }
  ...
  async function validateAdminSession() {
    ...
    try {
      ...
      currentAdmin = await supabaseAuth('user', 'GET');
      return true;
    } catch (error) {
      console.warn('Secure session expired or invalid:', error.message);
      clearSession();
      return false;
    }
  }
  ...
  async function signOutAdmin() {
    try { if (hasRemote() && getAccessToken()) await supabaseAuth('logout', 'POST'); }
    catch (error) { console.warn('Remote logout warning:', error.message); }
    clearSession();
  }
  ...
  async function supabaseRequest(path, method = 'GET', body, prefer = 'return=representation') {
    ...
    await ensureFreshSession();
    const token = getAccessToken() || backend.supabaseAnonKey;
    const headers = {
  ...
  async function fetchRemoteBookingsDirect() {
    if (!hasRemote()) return [];
    await ensureFreshSession();
    const token = getAccessToken() || backend.supabaseAnonKey;
    const columns = [
      'id', 'dateCreated', 'name', 'phone', 'email', 'preferredCommunication', 'altContact',
      'eventType', 'eventDate', 'guests', 'location', 'package', 'budget', 'message',
      'status', 'priority', 'notes', 'quoteAmount', 'assignedTo'
    ].join(',');
    const response = await fetch(`${apiBase()}/rest/v1/bookings?select=${columns}`, {
      method: 'GET',
      headers: {
        apikey: backend.supabaseAnonKey,
        Authorization: `Bearer ${token}`,
    ...
    const text = await response.text();
    if (!response.ok) throw new Error(`Bookings read failed: ${response.status} ${text}`);
    const rows = text ? JSON.parse(text) : [];
    if (!Array.isArray(rows)) return [];
    rows.sort((a, b) => new Date(b.dateCreated || b.eventDate || 0) - new Date(a.dateCreated || a.eventDate || 0));
    return rows;
  }

  async function refreshRemoteBookings() {
    if (!hasRemote()) return false;
    if (bookingReloadPromise) return bookingReloadPromise;
    bookingsLoadState = 'loading';
    bookingReloadPromise = (async () => {
      try {
        const rows = await fetchRemoteBookingsDirect();
        setBookings(rows);
        bookingsLoadState = 'loaded';
        return true;
      } catch (error) {
        bookingsLoadState = 'error';
        console.error('Remote booking load failed:', error.message);
        return false;
      } finally {
        bookingReloadPromise = null;
      }
    })();
    return bookingReloadPromise;
  }
  async function refreshRemoteConfig() {
    ...
    } catch (error) { ok = false; console.warn('Remote site settings refresh failed:', error.message); }
    try {
      const pricingRows = await supabaseRequest('quote_pricing?id=eq.main&select=pricing');
      if (Array.isArray(pricingRows) && pricingRows[0] && pricingRows[0].pricing) write(keys.quotePricing, normalisePricing(pricingRows[0].pricing));
    } catch (error) { ok = false; console.warn('Remote pricing refresh failed:', error.message); }
    try {
    ...
    } catch (error) { ok = false; console.warn('Remote banking refresh failed:', error.message); }
    try {
      const galleryRows = await supabaseRequest('gallery_items?select=id,title,category,image&order=created_at.desc');
      if (Array.isArray(galleryRows)) write(keys.gallery, galleryRows.length ? galleryRows : read(keys.gallery, defaults.gallery || []));
    } catch (error) { ok = false; console.warn('Remote gallery refresh failed:', error.message); }
    return ok;
  }
  async function upsertRemote(table, body) {
    if (!hasRemote()) return false;
    try {
      await supabaseRequest(`${table}?on_conflict=id`, 'POST', body, 'resolution=merge-duplicates,return=minimal');
      return true;
    } catch (firstError) {
      console.warn(`Remote ${table} upsert failed, trying update fallback:`, firstError.message);
      if (!body || !body.id) return false;
      const { id, ...changes } = body;
      try {
        await supabaseRequest(`${table}?id=eq.${encodeURIComponent(id)}`, 'PATCH', changes, 'return=minimal');
        return true;
      } catch (secondError) {
        console.warn(`Remote ${table} update fallback failed:`, secondError.message);
        return false;
      }
    }
  }
  ...
  function initDefaults() {
    ...
    if (!localStorage.getItem(keys.bookings)) write(keys.bookings, hasRemote() ? [] : (defaults.sampleBookings || []));
  }
  function getBookings() { return read(keys.bookings, []); }
  function setBookings(bookings) { write(keys.bookings, bookings); }
  function getPricing() {
    const pricing = normalisePricing(read(keys.quotePricing, defaults.quotePricing || {}));
    write(keys.quotePricing, pricing);
    return pricing;
  }
  ...
  function setLoginModeText() {
    const note = document.getElementById('loginModeNote');
    const emailField = document.getElementById('emailField');
    const submitBtn = document.getElementById('loginSubmitBtn');
    if (!note || !emailField || !submitBtn) return;
    if (hasRemote()) {
      emailField.classList.remove('hidden');
      submitBtn.textContent = 'Open Owner Portal';
      note.innerHTML = 'Use the owner email and password to continue.';
    } else {
      emailField.classList.add('hidden');
      submitBtn.textContent = 'Open Owner Portal';
      note.innerHTML = 'Use the presentation password for offline testing.';
    }
  }
  async function checkAuth() {
    setLoginModeText();
    if (hasRemote()) {
      if (await validateAdminSession()) await showDashboard();
      return;
    }
    if (sessionStorage.getItem(keys.localAuth) === 'yes') showDashboard();
  }
    ...
    form.addEventListener('submit', async event => {
      ...
      try {
        ...
        if (hasRemote()) {
          ...
          await showDashboard();
        } else if (password === (defaults.adminPassword || 'nick2026')) {
          sessionStorage.setItem(keys.localAuth, 'yes');
          showDashboard();
        } else {
        ...
        button.textContent = 'Open Owner Portal';
      }
    });
    document.getElementById('logoutBtn').addEventListener('click', async () => { await signOutAdmin(); location.reload(); });
  }
  function initTabs() {
  ...
  function openTab(id) {
    ...
    if (id === 'bookings' && hasRemote()) {
      refreshRemoteBookings().then(() => renderAll());
    }
  ...
  function renderBookings() {
    ...
    // If Supabase is connected but local booking cache is still empty, force one automatic online load.
    if (hasRemote() && bookingsLoadState === 'idle' && !getBookings().length) {
      bookingsLoadState = 'loading';
      table.innerHTML = '<tr><td colspan="7">Loading booking records...</td></tr>';
      refreshRemoteBookings().then(() => renderAll());
      return;
    }
    ...
    if (hasRemote() && bookingsLoadState === 'loading' && !bookings.length) {
      table.innerHTML = '<tr><td colspan="7">Loading booking records...</td></tr>';
      return;
    }
    ...
    if (!bookings.length) {
      const msg = hasRemote() && bookingsLoadState === 'error'
        ? 'Booking records could not be loaded. Log out, log in again, and check Supabase connection.'
        : 'No matching bookings found.';
      table.innerHTML = `<tr><td colspan="7">${sanitize(msg)}</td></tr>`;
      return;
    }
    ...
    table.innerHTML = bookings.map(b => `
      <tr>
        ...
        <td>${sanitize(b.eventType)}<br><small>${sanitize(b.package || 'Custom / not selected')} • ${sanitize(b.location)}</small></td>
  ...
  function updateBooking(id, changes) {
    ...
    if (hasRemote()) supabaseRequest(`bookings?id=eq.${encodeURIComponent(id)}`, 'PATCH', changes, 'return=minimal').catch(error => console.warn('Remote update failed:', error.message));
    renderAll();
  }
  ...
  function initBookingControls() {
    ...
    document.getElementById('bookingTable').addEventListener('change', e => { const id = e.target.dataset.id; if (e.target.dataset.action === 'status') updateBooking(id, { status: e.target.value }); if (e.target.dataset.action === 'quote') { const amount = e.target.value === '' ? null : e.target.value; updateBooking(id, { quoteAmount: amount }); } });
    document.getElementById('bookingTable').addEventListener('blur', e => { if (e.target.dataset.action === 'quote') { const amount = e.target.value === '' ? null : e.target.value; updateBooking(e.target.dataset.id, { quoteAmount: amount }); } }, true);
    document.getElementById('bookingTable').addEventListener('click', e => { const action = e.target.dataset.action; const id = e.target.dataset.id; if (!action) return; const bookings = getBookings(); const booking = bookings.find(b => b.id === id); if (action === 'view') showBooking(id); if (action === 'delete' && confirm('Delete this booking?')) { setBookings(bookings.filter(b => b.id !== id)); if (hasRemote()) supabaseRequest(`bookings?id=eq.${encodeURIComponent(id)}`, 'DELETE', null, 'return=minimal').catch(error => console.warn('Remote delete failed:', error.message)); renderAll(); } if (action === 'whatsapp' && booking) window.open(bookingWhatsapp(booking), '_blank'); });
    document.getElementById('addBookingBtn').addEventListener('click', showAddBookingForm);
  }

  function showAddBookingForm() {
    currentBookingId = null;
    document.getElementById('modalContent').innerHTML = `
      <p class="eyebrow">Manual booking capture</p><h1>Add booking</h1>
      <form id="manualBookingForm" class="modal-form">
        <div class="detail-grid form-grid">
          <label>Client full name<input name="name" required placeholder="Client name" /></label>
          <label>Phone number<input name="phone" required placeholder="076 000 0000" /></label>
          <label>Email<input name="email" type="email" placeholder="client@example.com" /></label>
          <label>Preferred communication<select name="preferredCommunication"><option>WhatsApp</option><option>Call</option><option>Email</option></select></label>
          <label>Alternative contact<input name="altContact" placeholder="Optional" /></label>
          <label>Event type<input name="eventType" required placeholder="Wedding / Corporate / Birthday" /></label>
          <label>Event date<input name="eventDate" type="date" /></label>
          <label>Guests<input name="guests" placeholder="120" /></label>
          <label>Location<input name="location" placeholder="Tzaneen" /></label>
          <label>Package<input name="package" placeholder="Signature Experience" /></label>
          <label>Budget<input name="budget" placeholder="R25 000 - R40 000" /></label>
          <label>Priority<select name="priority"><option>Normal</option><option>High</option><option>Medium</option><option>Low</option></select></label>
          <label class="full">Client request<textarea name="message" rows="4" placeholder="What does the client need?"></textarea></label>
        </div>
        <div class="row-actions" style="margin-top:14px"><button class="btn btn-gold" type="submit">Save Booking</button><button class="btn btn-outline" type="button" id="cancelManualBookingBtn">Cancel</button></div>
      </form>`;
    document.getElementById('bookingModal').classList.remove('hidden');
  }
  ...
  async function createManualBooking(data) {
    const booking = {
      id: `EWN-${Date.now().toString().slice(-6)}`,
    ...
    if (hasRemote()) {
      try {
        await supabaseRequest('bookings', 'POST', booking, 'return=minimal');
        systemNotice('', 'info');
        await refreshRemoteBookings();
        renderAll();
      } catch (error) {
        systemNotice('Manual booking could not be saved online. Check Supabase login and schema.', 'error');
      }
    }
  }
  ...
  function initModal() {
    ...
    document.getElementById('bookingModal').addEventListener('click', e => { if (e.target.id === 'bookingModal' || e.target.id === 'cancelManualBookingBtn') e.currentTarget.classList.add('hidden'); });
    document.getElementById('bookingModal').addEventListener('submit', async e => {
      if (e.target.id !== 'manualBookingForm') return;
      e.preventDefault();
      await createManualBooking(Object.fromEntries(new FormData(e.target).entries()));
  ...
  function populateQuoteSelects() {
    ...
    ['serviceLevels','decorLevels','cateringOptions','extras'].forEach(group => { if (!Array.isArray(pricing[group])) pricing[group] = []; });
  ...
  function renderPricingForm() {
    ...
    if (!form) return;
    const pricing = getPricing();
    const section = (title, group, enabled = false) => `
      <div class="price-section-head"><h3>${title}</h3><button class="icon-btn" type="button" data-price-add="${group}">Add item</button></div>
      ${((pricing[group] || []).map((item, i) => priceRow(group, item, i, enabled)).join('') || '<p class="muted">No items yet. Click Add item.</p>')}`;
    form.innerHTML = `
      ${section('Service levels', 'serviceLevels')}
      ${section('Decor levels', 'decorLevels')}
      ${section('Catering support', 'cateringOptions')}
      ${section('Extra charges', 'extras', true)}
  ...
  function collectPricingFromForm() {
    ...
    if (perGuest) pricing.perGuestCoordination = Number(perGuest.value || 0);
    return pricing;
  }
  function initPricing() {
    ...
    form.addEventListener('click', e => {
      const btn = e.target.closest('[data-price-add], [data-price-remove]');
      if (!btn || !form.contains(btn)) return;
      const addGroup = btn.dataset.priceAdd;
      const removeGroup = btn.dataset.priceRemove;
      if (!addGroup && !removeGroup) return;
      const pricing = collectPricingFromForm();
      if (addGroup) {
        if (!pricing[addGroup]) pricing[addGroup] = [];
        pricing[addGroup].push({ id: `${addGroup}-${Date.now()}`, label: 'New item', amount: 0, ...(addGroup === 'extras' ? { enabled: true } : {}) });
      }
      if (removeGroup) {
        const index = Number(btn.dataset.priceIndex);
        if (pricing[removeGroup]) pricing[removeGroup].splice(index, 1);
      }
      setPricing(pricing);
      renderPricingForm();
      populateQuoteSelects();
      document.getElementById('pricingSaved').textContent = 'Unsaved price changes. Click Save Price Settings.';
    });
    document.getElementById('savePricingBtn').addEventListener('click', async () => {
      const pricing = collectPricingFromForm();
      setPricing(pricing); populateQuoteSelects();
      const savedRemote = await upsertRemote('quote_pricing', { id: 'main', pricing, updated_at: new Date().toISOString() });
      document.getElementById('pricingSaved').textContent = hasRemote() ? (savedRemote ? 'Price settings saved.' : 'Price settings saved locally, but online save failed.') : 'Price settings saved.';
    });
    document.getElementById('resetPricingBtn').addEventListener('click', () => { if (!confirm('Reset prices to default?')) return; setPricing(defaults.quotePricing || {}); renderPricingForm(); populateQuoteSelects(); document.getElementById('pricingSaved').textContent = 'Default prices restored. Click Save Price Settings to sync online.'; });
  }
  ...
  function initSettings() {
    ...
    form.addEventListener('submit', async event => {
      ...
      const savedRemote = await upsertRemote('site_settings', { id: 'main', settings: data, updated_at: new Date().toISOString() });
      document.getElementById('settingsSaved').textContent = hasRemote() ? (savedRemote ? 'Website details saved locally and to Supabase.' : 'Website details saved locally, but online save failed.') : 'Website details saved. Refresh the public website to see changes.';
    });
  }
  ...
  function initBanking() {
    ...
    form.addEventListener('submit', async event => {
      ...
      const data = Object.fromEntries(new FormData(form).entries());
      setBanking(data);
      const savedRemote = await upsertRemote('banking_details', { id: 'main', banking: data, updated_at: new Date().toISOString() });
      document.getElementById('bankingSaved').textContent = hasRemote() ? (savedRemote ? 'Banking details saved locally and to Supabase.' : 'Banking details saved locally, but online save failed.') : 'Banking details saved. Quote PDF and quote text will use these details.';
    });
  }
  ...
  function setGalleryPreview(image) {
  ...
  function renderGallery() {
    ...
    if (!container) return;
    container.innerHTML = gallery.length ? gallery.map((item, index) => `
      <article class="gallery-admin-card">
        <div class="thumb" style="background-image:url('${sanitize(item.image)}')"></div>
        <div>
          <h3>${sanitize(item.title)}</h3>
          <p>${sanitize(item.category)}</p>
          <button class="icon-btn danger" data-gallery-delete="${index}">Remove</button>
        </div>
      </article>
    `).join('') : '<p class="empty-state">No gallery images yet. Add the first photo above.</p>';
  }
  ...
  function initGallery() {
    ...
    const saved = document.getElementById('gallerySaved');
    const clearBtn = document.getElementById('clearGalleryImageBtn');
    const container = document.getElementById('adminGallery');
    ...
    if (fileInput) {
      fileInput.addEventListener('change', async () => {
        ...
        if (!file) return;
        try {
          pendingGalleryImage = await compressImageFile(file);
          if (urlInput) urlInput.value = '';
          setGalleryPreview(pendingGalleryImage);
        } catch (error) {
          pendingGalleryImage = '';
          setGalleryPreview('');
    ...
    if (urlInput) {
      urlInput.addEventListener('input', () => {
        if (urlInput.value.trim()) {
          ...
          setGalleryPreview(urlInput.value.trim());
        } else {
          setGalleryPreview('');
        }
      });
    ...
    form.addEventListener('submit', async event => {
      ...
      const image = pendingGalleryImage || (raw.image || '').trim();
      if (!image) {
        saved.textContent = 'Please upload a photo or paste an image link.';
        return;
      }
      const data = { title: (raw.title || '').trim(), category: (raw.category || '').trim(), image };
      if (!data.title || !data.category) {
        saved.textContent = 'Please add the image title and category.';
        return;
      }

      let itemToStore = data;
      try {
        if (hasRemote()) {
          const inserted = await supabaseRequest('gallery_items', 'POST', data);
          if (Array.isArray(inserted) && inserted[0]) itemToStore = inserted[0];
        }
        const gallery = read(keys.gallery, defaults.gallery || []);
        gallery.unshift(itemToStore);
        write(keys.gallery, gallery);
        form.reset();
        pendingGalleryImage = '';
        setGalleryPreview('');
        renderGallery();
        saved.textContent = 'Gallery image added.';
      } catch (error) {
        saved.textContent = `Could not save gallery image online: ${error.message}`;
      }
    });

    container.addEventListener('click', async e => {
      const index = e.target.dataset.galleryDelete;
      if (index === undefined) return;
      const gallery = read(keys.gallery, defaults.gallery || []);
      const item = gallery[Number(index)];
      if (!item) return;
      if (!confirm(`Remove "${item.title || 'this image'}" from the gallery?`)) return;

      try {
        if (hasRemote() && item.id) {
          await supabaseRequest(`gallery_items?id=eq.${encodeURIComponent(item.id)}`, 'DELETE', null, 'return=minimal');
        }
        gallery.splice(Number(index), 1);
        write(keys.gallery, gallery);
        renderGallery();
      } catch (error) {
        alert(`Could not remove this gallery image online: ${error.message}`);
      }
    });
  }
  function exportCsv() {
  ...
  function safeRender(name, fn) {
    try { fn(); } catch (error) { console.error(`${name} render failed:`, error.message); }
  }
  function renderAll() {
    safeRender('Bookings', renderBookings);
    safeRender('Metrics', renderMetrics);
    safeRender('Recent bookings', renderRecent);
    safeRender('Pipeline', renderPipeline);
    safeRender('Gallery', renderGallery);
    safeRender('Pricing', renderPricingForm);
    safeRender('Website details', renderSettingsForm);
    safeRender('Banking', renderBankingForm);
    safeRender('Quote selects', populateQuoteSelects);
  }
